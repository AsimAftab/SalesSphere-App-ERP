...
class EditVisitNoteDetailPage extends ConsumerStatefulWidget {
  const EditVisitNoteDetailPage({required this.id, this.initial, super.key});

  final String id;

  /// Optional starting note passed via `extra` when navigating from
  /// the list — saves a re-fetch on first paint.
  final VisitNote? initial;

  @override
  ConsumerState<EditVisitNoteDetailPage> createState() =>
      _EditVisitNoteDetailPageState();
}

class _EditVisitNoteDetailPageState
    extends ConsumerState<EditVisitNoteDetailPage> {
  ...
  Future<void> _hydrate() async {
    VisitNote? note;
    try {
      note = await ref.read(visitNoteByIdProvider(widget.id).future);
    } on Object catch (_) {
  ...
  void _cancelEdit() {
    final saved =
        ref.read(visitNoteByIdProvider(widget.id)).value ?? widget.initial;
  ...
  Widget build(BuildContext context) {
    ...
    ref.watch(visitNoteByIdProvider(widget.id));
      ...
      child: Scaffold(
        ...
        body: Center(
          child: Padding(
            ...
            child: Text(
              "Couldn't load this visit note — it may have been removed.",
