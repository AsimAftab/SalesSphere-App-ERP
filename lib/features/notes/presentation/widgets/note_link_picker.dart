...
/// fields on the note so the list page can render without
/// resolving cross-feature lookups per row.
@immutable
class NoteLinkSelection {
  const NoteLinkSelection({
    required this.type,
    required this.id,
...
typedef NoteLinkPickerResult = ({NoteLinkSelection? value, bool cleared});
...
Future<NoteLinkPickerResult?> showNoteLinkPicker(
  BuildContext context, {
  NoteLinkSelection? current,
}) {
  return showModalBottomSheet<NoteLinkPickerResult>(
    context: context,
    isScrollControlled: true,
    backgroundColor: Colors.transparent,
    builder: (_) => _NoteLinkPickerSheet(current: current),
  );
}
...
class _NoteLinkPickerSheet extends StatelessWidget {
  const _NoteLinkPickerSheet({this.current});

  final NoteLinkSelection? current;

  int get _initialTabIndex {
    final c = current;
    if (c == null) return 0;
    return switch (c.type) {
      NoteLinkType.party => 0,
      NoteLinkType.prospect => 1,
      NoteLinkType.site => 2,
    };
  }
  ...
  Widget build(BuildContext context) {
    final c = current;
    final partySelectedId = c != null && c.type == NoteLinkType.party
        ? c.id
        : null;
    final prospectSelectedId = c != null && c.type == NoteLinkType.prospect
        ? c.id
        : null;
    final siteSelectedId = c != null && c.type == NoteLinkType.site
        ? c.id
        : null;
            ...
            children: <Widget>[
              ...
              if (c != null) ...<Widget>[
                ...
                ListTile(
                  ...
                  onTap: () => Navigator.of(
                    context,
                  ).pop<NoteLinkPickerResult>((value: null, cleared: true)),
                ),
              ],
            ],
          ),
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                return _LinkRow(
                  ...
                  onTap: () => Navigator.of(context).pop<NoteLinkPickerResult>((
                    value: NoteLinkSelection(
                      type: NoteLinkType.party,
                      id: party.id,
