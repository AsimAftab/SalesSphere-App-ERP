...
class AuthApi {
  ...
  /// Changes the signed-in user's password. The backend invalidates the
  /// user's other sessions and returns a human-readable confirmation under
  /// `data.message`. Returns that message so the UI can echo it verbatim.
  Future<String> changePassword(ChangePasswordRequestDto req) async {
    final response = await _dio.post<Map<String, dynamic>>(
      Endpoints.changePassword,
      data: req.toJson(),
    );
    final message = _unwrap(response.data)['message'];
