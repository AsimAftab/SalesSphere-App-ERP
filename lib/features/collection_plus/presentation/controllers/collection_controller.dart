...
// `collection_providers.dart` re-exports `collectionPlusRepositoryProvider` so
...
/// Reads stay on `collectionPlusListVisibleProvider` /
/// `collectionPlusByIdProvider`.
@riverpod
class CollectionPlusController extends _$CollectionPlusController {
  ...
  Future<CollectionPlus> addCollection({
    ...
    try {
      ...
      ref.read(collectionPlusListProvider.notifier).prependLocal(e.collection);
      rethrow;
    } finally {
      link.close();
    }
  ...
  Future<void> deleteCollection(String id) async {
    final link = ref.keepAlive();
    try {
      await ref.read(collectionPlusRepositoryProvider).deleteCollection(id);
      ref.read(collectionPlusListProvider.notifier).removeLocal(id);
    } finally {
      link.close();
    }
  }
